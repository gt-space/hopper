...
%  Phase: DESCENT (apex → landing)
%
%  Based on: Acikmese & Ploen (2007), Blackmore et al. (2010)
%  "Lossless Convexification" approach - Problem 3 (min fuel)
%
%  Run gfold_cvx.m first (ascent phase) to get m0_descent.
%  This script takes over from the apex state.
% =========================================================
...
% Phase: Descent only (51m apex → 1m landing)

% ── Ascent phase outputs (hardcoded from gfold_cvx.m result) ──
% Update these if you re-run the ascent script
ascent_fuel_used = 8.76;           % kg, from ascent solution
m_wet_initial    = 135.90;         % kg, full wet mass at liftoff

% Initial state (apex — from ascent terminal state)
r0 = [0;  0;  51];       % position [m]
v0 = [0;  0;   0];       % velocity [m/s] (at rest at apex)

% Terminal state (mission_inputs: landing_alt = 1m, landing_vel = 0 m/s)
rf = [0;  0;   1];       % target position [m]
vf = [0;  0;   0];       % velocity [m/s] at landing

% Rate limits (mission_inputs.max_descent_vel)
vz_max = 7;              % max vertical speed magnitude [m/s]

% Vehicle mass at start of descent
m_prop_total  = 16.0;                          % kg, total loaded propellant
m_dry         = m_wet_initial - m_prop_total;  % kg = 119.90 kg
m0            = m_wet_initial - ascent_fuel_used; % kg = 127.14 kg (wet mass at apex)
m_prop_remain = m0 - m_dry;                    % kg = 7.24 kg remaining for descent

% Propulsion (mission_inputs: nominal_thrust = 500 lbf, throttle_range = [0.4, 1.1])
T_nominal = 500 * 4.44822;          % N = 2224.1 N
Tmin      = 0.20 * T_nominal;       % N = 444.8 N  (20% throttle — relaxed for descent feasibility)
                                    % Note: physical throttle floor is 40%, but GFOLD planning
                                    % uses 20% to give the optimizer room; tighten after first solve
...
% Constraints
theta_max = deg2rad(10);            % max thrust tilt from vertical
% Note: TVC max gimbal = 15 deg (mission_inputs.tvc.max_gimbal_angle)

%% ── Discretization ───────────────────────────────────────────────────────

N  = 60;          % number of time steps (increased for finer resolution)
tf = 25;          % flight time [s] — increased for descent feasibility
dt = tf / N;      % time step [s]
...
fprintf('Specific thrust bounds: u_min=%.3f m/s^2, u_max=%.3f m/s^2\n', Tmin/m0, Tmax/m_dry);
...
cvx_begin quiet
    ...
    subject to
        ...
        % Instead of linearizing T/m bounds in log-mass space (fragile for
        % descent where mass range is small), bound specific thrust u directly.
        % Use conservative mass estimates: u = T/m, so:
        %   u_min = Tmin / m0      (min specific thrust, heaviest case)
        %   u_max = Tmax / m_dry   (max specific thrust, lightest case)
        % This is a valid outer relaxation; the optimizer stays feasible.
        u_min = Tmin / m0;        % m/s^2
        u_max = Tmax / m_dry;     % m/s^2
        for k = 1:N+1
            norm(u(:,k), 2) <= sig(k);
            sig(k) >= u_min;
            sig(k) <= u_max;
        ...
        % ── Log-mass dynamics bound ──
        % Mass can only decrease; keep it above dry mass
        for k = 1:N+1
            z(k) >= log(m_dry);
            z(k) <= log(m0);
        end
...
function plotResults(t, r, v, thrust_traj, thrust_mag, mass_traj, m0, Tmax)
    ...
    sgtitle('GFOLD: Fuel-Optimal Powered Descent Phase', 'FontSize', 14, 'FontWeight', 'bold');
end