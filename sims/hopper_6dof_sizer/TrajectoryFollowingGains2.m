function [GainTable] = TrajectoryFollowingGains2(Sf,Q,R,Trajectory,A,B,tgrid,unom,params,Fref)

% x = [X Y Z Vx Vy Vz P Q R q0 q1 q2 q3]
% u = [T delta_p delta_y F_rcs]

Eta   = Trajectory;      % N x 13
tgrid = tgrid(:);
N     = numel(tgrid);
t0    = tgrid(1);
tf    = tgrid(end);
...
%% defect: d = xref_dot - f(xref, uref)

d = Etadot - Fref;   % N x 13

%% solve Riccati for S backwards
[tS,Svec] = ode45(@(t,S) Sdot(t,S,A,B,Q,R,tgrid), [tf t0], Sf(:));
