function IN = mc_inputs(scenario)
%  GLOBAL CONSTANTS
...
IN.propulsion.oxidizer_mass = scenario.ox_mass; % kg -> internal
IN.propulsion.fuel_mass = scenario.fuel_mass; % kg -> internal
...
IN.propulsion.eta_cstar = scenario.cstar;
...
IN.off_axis_y = scenario.engine_off_axis_y; % m  % Body Frame
IN.off_axis_z = scenario.engine_off_axis_z;% m 
...
IN.tanks.lateral_damping = scenario.slosh_lateral_damping;
IN.tanks.axial_damping = scenario.slosh_axial_damping;

%  TVC
IN.tvc.max_gimbal_angle = deg2rad(15); % ?
IN.tvc.max_gimbal_rate = deg2rad(scenario.tvc_actuator_rate_limit); % rad/s ?
IN.tvc.voltage = 24; % V
IN.tvc.current = 3; % A
IN.tvc.actuator_latency = scenario.tvc_actuator_latency; % s

% THROTTLE VALVE
IN.throttle_valve.voltage = 24; % V
IN.throttle_valve.current = 1; % A
IN.throttle_valve.mass = 2.3; % kg
IN.throttle_valve.rate_limit = scenario.throttle_rate_limit * 4.44822; % N
IN.throttle_valve_latency = scenario.throttle_latency; % s
