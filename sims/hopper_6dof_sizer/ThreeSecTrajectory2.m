...
Tcheck = 3; % 3 second check period
...
t_end = Tup + Thover + Tdown + Tterm + Tcheck;
t = (0:dt:t_end)';
...
for k = 1:length(t)
    ...
    if tk < Tcheck
        ...
        z(k) = 0; % Keeps vertical position at 0
        T_cmd(k) = 0; % Make sure engines are off

    elseif tk < Tup + Tcheck
    ...
    elseif tk < Tup + Thover + Tcheck

        % HOVER
        z(k) = zf;
        T_cmd(k) = m(k)*g;

    elseif tk < Tup + Thover + Tdown + Tcheck

        % DESCENT
        s = (tk-(Tcheck+Tup+Thover))/Tdown;
        s = min(max(s,0),1);
