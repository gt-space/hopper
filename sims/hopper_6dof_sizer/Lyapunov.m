...
% 
% for i = 1:13
% omega_n(i) = sqrt(real(p(i)).^2+imag(p(i)).^2);
% zeta(i) = -real(p(i))./omega_n(i)';
% end
% 
% sys = tf(omega_n(1), [1 2*zeta(1)*omega_n(1)  omega_n(1)^2]);
% 
% figure
% bode(sys)
% %%
...
%% =========================
%  RESPONSE / BODE FROM POLES
%  Chapter 8 style interpretation
%  Uses canonical 1st and 2nd order systems based on pole locations
%  =========================

close all

% pick one time index to analyze
kplot = 1;                  % change as needed
p = poles(:,kplot);

% remove numerical duplicates later when pairing
used = false(length(p),1);
tol_pair = 1e-6;
