function IN = mc_inputs(scenario_params)
    % Handle Nominal Mode (no inputs passed) by setting default baseline parameters
    if nargin == 0 || isempty(scenario_params)
        scenario_params.ox_mass = 16;
        scenario_params.fuel_mass = 10;
        scenario_params.cstar = 0.85;
        scenario_params.engine_off_axis_y = 0;
        scenario_params.engine_off_axis_z = 0;
        scenario_params.slosh_lateral_damping = 0.02;
        scenario_params.slosh_axial_damping = 0.1;
        scenario_params.tvc_actuator_rate_limit = deg2rad(30);
        scenario_params.tvc_actuator_latency = 0.01;
        scenario_params.throttle_rate_limit = 90;
        scenario_params.throttle_latency = 0.01;
        scenario_params.uwind = 0;
        scenario_params.vwind = 0;
        scenario_params.mass_factor = 1.0;
    end
    ...
    IN.propulsion.fuel = 'Dodecane'; % kerosene
    ...
    IN.tvc.max_gimbal_rate = scenario_params.tvc_actuator_rate_limit;
    IN.tvc.voltage = 24; % V
    IN.tvc.current = 3; % A
